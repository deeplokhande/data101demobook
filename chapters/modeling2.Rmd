...
Before we proceed to the next section, please look at the snippet of the *housing_price.csv* dataset, which we will be using for predicting the *median_house_values* attribute based on various other attributes provided in the dataset, using different prediction models.

```{r,echo=FALSE,error=FALSE,warning=FALSE}
library(kableExtra)
housing<-read.csv("https://raw.githubusercontent.com/deeplokhande/data101demobook/main/files/dataset/housing_prices.csv") #web load
temp<-knitr::kable(housing[sample(nrow(housing),10),], caption = 'Snippet of Housing Price Dataset',booktabs=TRUE) 
...
Now lets look at an example of prediction by the random forest model using the *randomForest()* function present in the `randomForest` library package.

Thus following the 4 step method of prediction for predicting a numerical attribute "median_house_value" using the randomForest() function.
