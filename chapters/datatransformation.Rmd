# Data Frames & Transformation. {#newcolumn}
...
library(knitr)
# moody<-read.csv("../files/dataset/moody2020b.csv") #static Load
moody<-read.csv("https://raw.githubusercontent.com/deeplokhande/data101demobook/main/files/dataset/SmallMoody.csv") #web load
knitr::kable(
  head(moody), caption = 'Small subset of Moody Dataset',
  booktabs = TRUE
)
...
# moody<-read.csv("../files/dataset/moody2020b.csv") #static Load
library(knitr)
moody<-read.csv("https://raw.githubusercontent.com/deeplokhande/data101demobook/main/files/dataset/SmallSt.csv") #web load
```


```{r tut=FALSE,echo=F}
knitr::kable(
  head(moody), caption = 'Small dataset of students information',
  booktabs = TRUE
)
...
```{r}
#Local Import
# moody_df<-read.csv("../files/dataset/SmallMoody.csv")
# student_df<-read.csv("../files/dataset/SmallSt.csv")
...
There are also arguments like `by.x and by.y` which correspond to indexing based on one of the column from the left(first) or right(second) datasets respectively.
...
There is an R package called `sqldf` that allows you to use SQL commands to extract data from an R data frame.  We will not use this package in the examples but look at a way the operations in SQL translate to basic R commands that we have studied in previous chapter \@ref(revcommands).
...
**Note**: In some cases, where you want to use the subsetted column with other function, e.g. `mean(subsetted_column)` you must not use the `drop=F` attribute, since it will result in error.


2. _Subset multiple column._
...
3. You can get all the columns in the subset, by keeping the space after the comma blank. This gives the complete set of columns.
...
1. _Subsetting based on single condition._
...
2. _Subsetting based on multiple conditions._
...
3. _Subset based on multiple values._
...
4. _Subset based on a partial/complete text/character._
