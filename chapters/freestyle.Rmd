...
- We will begin with what we call, the free style data exploration. We call it free style, since we are not going to use any sophisticated libraries, but rather just 7 basic of commands of R. And with these 7 commands we will be able to do quite a bit, more than you would ever guess in fact!
...
You will see now that with just 7 commands (4 plots plus 3 commands slicing and dicing data frames) we can achieve **Instant gratification through  data exploration!**

It is amazing how much data exploration can be done, just knowing 7 R commands. You can learn one command a day, or an hour. And be ready to show off your skills and impress your audience even with such small arsenal of tools at your disposal.

Through this section we will use our "crown" inital example, of synthetically generate data describing mysterious methods of grading used by an excentric Professor Moody. We have been using different versions of this *data puzzle* over the 6 years of teaching data 101. Data is different but narrative is always the same:
...
Score and grade are self explanatory. Participation is supposedly measuring students participation in class. We do not know whether higher particpation would necessarilly be postive, since Professor Moody's mood changes from year to year and he may be annoyed by students who are too active and bother him too much. Who knows? We have to find out.

Attribute "questions" has several values “always”, “frequently”, “sometimes” and “never”. So does the attribute "texting". In our data set there are students who are are always texting and who never ask any questions. Oh, yes, and some students particpation index is almost zero. Guess what grade are they getting? F, you probably guess. Well, but abouth their score.It should matter probably. At least to some degree!


Grading rules, which Professor Moody applies each year are different. The objective of our free style data exploration is to find some leads/hypothese which would help us direct students what they should do to get a good grade in his class. 
...
- Come to class as often as you can?  Presumably imporving particpation idex?
...
As illustrated by the boxplot, there are significant overlaps between successive grades.  This disproves that there is a determininstic function between score and grade. At least it is not always a function. If your score falls in certain "grey" areas you may get either one of two grades (A or B, B or C, C or D, D or F).  And we do not know what is this aditional "decider" in such case when score falls into this grey area. 
...
For example,we want to know if asking questions “matters” for the grade?   This can be validated by comparing barplots of grade distribution for different values of attribute "questions".You can either do it by applying the mossaic plot which allows for two-dimesnional representation of data and allows to create multicolored table for grade x questions to eyeball if values of attribute "questions" matter for values of attribute "grade".
...
# Notice that you can modify these barplots graphs and replace the value of moody$questions attributes from "always" to "sometimes" or "never" and see impact these new slices have on the grade distribution. Just change to code above and run it. You can also change the moody$questions attribute and replace it with the moody$texting attribute and its different values. Thus you can run 6 different barplots using the code above and see how Grade distributon is affected for each of these 6 cases.
```
...
Similarilly, does asking questions often imply higher score? Or it does not affect the score but affects the grade through the grading rules of Professor Moody.
...
TThen we can examine how participation influences grades in these grey areas of score. Our hypothesis is that higher participation would probably offer better odds for higher grade.
We can run the following command for different values of q. 

Let q be a threshold of particpation which we want to test. May be if particpation is higher than q, higer grade (from the two possible grades in the grey area of score) is given, while if partcipation is lower than q, it works againts a student, who then gets lower grade?
...
Run the following command for different values of q. We will only show it for the grey interval between A and B. The same process can be repeated for othe grey areas between B and C, C and D and D and F. In fact one can modify the code below by just replacing grade A and B with B and C respetively as well as 
...
Is there a critical value of q which clearly separates, say A's from B's?  It seems to be q=0.6 - but it is not a clear cut deterministic. Rather, higher value of particpation threshold, q increases probability (frequency) of getting As. 
...
This what we embedded in our data generation method:  particpation increases the chances of higher grade in the grey areas of score (border areas). 
...
Therefore we have discovered more or less the rule which guided generation of the Moody's data set and provided students with actionable intellignece how to increase chances of getting higher grade.
...
Our professor Moody's data puzzle has been traditional the first data puzzle we ask students to solve in data 101 class. Here are some examples of conlusions reached on different instances of professor Moody's data set in the past semesters.  Notice that attributes of Moody's data set in past may have been different (like "sleeping in class")
