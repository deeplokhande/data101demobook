...
**NOTE: Not all code chunks are converted to interactive, only the first create column example is converted.**

Text by Prof. Imielinski.
...
colyear <- cut(sample(18:22, nrow(mdy), replace=TRUE),breaks=4,labels=c( "Freshman", "Sophomore", "Junior", "Senior"))
...
names(mdy) # Initially dataframe has 7 columns
mdy$COL_YEAR <- colyear #Put syntax dataFrameName$columnHeaderName <- newColumn
names(mdy) # Now dataframe has 8 columns
```
...
mdy$badcol <- badcol #Throws Compatibility error. 
```
